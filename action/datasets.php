<?php
	...
	if($kelompok=="status"){
    	if($rentang=='Tahun'){
    	    ...
    	    $data = mysqli_query($koneksi,"SELECT count(id_aduan), status, DATE_FORMAT(date(waktu_kirim),'%Y-%m') FROM tb_aduan where date(waktu_kirim)>=date('".$from."') and date(waktu_kirim) <= date('".$to."') group by status, DATE_FORMAT(date(waktu_kirim),'%Y%m')");
        	foreach($data as $row){
    	    ...
    	    $data = mysqli_query($koneksi,"SELECT count(id_aduan), status, DATE_FORMAT(date(waktu_kirim),'%d-%m-%Y') FROM tb_aduan where date(waktu_kirim)>=date('".$from."') and date(waktu_kirim) <= date('".$to."') group by status, DATE_FORMAT(date(waktu_kirim),'%d-%m-%Y')");
        	foreach($data as $row){
	    ...
	    if($rentang=='Tahun'){
        	$data = mysqli_query($koneksi,"SELECT count(id_aduan), jenis, year(date(waktu_kirim)) FROM tb_aduan where year(date(waktu_kirim))>=year(date('".$from."')) and year(date(waktu_kirim)) <= year(date('".$to."')) group by jenis, year(date(waktu_kirim))");
        	foreach($data as $row){
        	...
        	$data = mysqli_query($koneksi,"SELECT count(id_aduan), jenis, DATE_FORMAT(date(waktu_kirim),'%Y-%m') FROM tb_aduan where date(waktu_kirim)>=date('".$from."') and date(waktu_kirim) <= date('".$to."') group by jenis, DATE_FORMAT(date(waktu_kirim),'%Y%m')");
        	foreach($data as $row){
    	    ...
    	    $data = mysqli_query($koneksi,"SELECT count(id_aduan), jenis, DATE_FORMAT(date(waktu_kirim),'%d-%m-%Y') FROM tb_aduan where date(waktu_kirim)>=date('".$from."') and date(waktu_kirim) <= date('".$to."') group by jenis, DATE_FORMAT(date(waktu_kirim),'%d-%m-%Y')");
        	foreach($data as $row){
