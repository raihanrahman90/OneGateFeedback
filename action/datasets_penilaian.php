<?php
	...
	if($kelompok=="rata-rata"){
	    ...
	    if($rentang=='Tahun'){
        	$data = mysqli_query($koneksi,"SELECT count(tb_aduan.id_aduan) as jumlah, penilaian, year(date(waktu_kirim)) FROM tb_aduan 
                                            inner join tb_penilaian on tb_penilaian.id_aduan = tb_aduan.id_aduan 
        	...
        	foreach($data as $row){
        	    $data_kembali["data"][$row['penilaian']][$row['year(date(waktu_kirim))']]=$row['jumlah'];
        	    if(!in_array($row['year(date(waktu_kirim))'], $data_kembali["label"])){
        	        array_push($data_kembali["label"], $row['year(date(waktu_kirim))']);
        	    }
        	}
    	} else if($rentang =='Bulan'){
        	$data = mysqli_query($koneksi,"SELECT count(tb_aduan.id_aduan) as jumlah, penilaian, DATE_FORMAT(date(waktu_kirim),'%Y-%m') FROM tb_aduan  
                                            inner join tb_penilaian on tb_penilaian.id_aduan = tb_aduan.id_aduan 
        	...
        	foreach($data as $row){
        	    $data_kembali["data"][$row['penilaian']][$row["DATE_FORMAT(date(waktu_kirim),'%Y-%m')"]]=$row['jumlah'];
        	    if(!in_array($row["DATE_FORMAT(date(waktu_kirim),'%Y-%m')"], $data_kembali["label"])){
        	        array_push($data_kembali["label"], $row["DATE_FORMAT(date(waktu_kirim),'%Y-%m')"]);
        	    }
        	}
    	} else {
    	    $data = mysqli_query($koneksi,"SELECT count(tb_aduan.id_aduan) as jumlah, penilaian, DATE_FORMAT(date(waktu_kirim),'%d-%m-%Y') FROM tb_aduan  
                                            inner join tb_penilaian on tb_penilaian.id_aduan = tb_aduan.id_aduan 
        	...
        	foreach($data as $row){
        	    $data_kembali["data"][$row['penilaian']][$row["DATE_FORMAT(date(waktu_kirim),'%d-%m-%Y')"]]=$row['jumlah'];
        	    if(!in_array($row["DATE_FORMAT(date(waktu_kirim),'%d-%m-%Y')"], $data_kembali["label"])){
        	        array_push($data_kembali["label"], $row["DATE_FORMAT(date(waktu_kirim),'%d-%m-%Y')"]);
        	    }
        	}
